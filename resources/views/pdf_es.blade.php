@php
    ...
    $jsonPath = resource_path("views/lang/es.json");
...
<body>
  ...
  <div class="section">
    <p>
      <strong>{{ translate_key('create.race', $translations) }}:</strong>
      {{ translate_key('races.' . normTexto($ficha->race->name), $translations, $ficha->race->name) }}
    </p>

    <p>
      <strong>{{ translate_key('create.subrace', $translations) }}:</strong>
      @if($ficha->subrace)
        {{ translate_key('subraces.' . normTexto($ficha->subrace->name), $translations, $ficha->subrace->name) }}
      @else
        {{ translate_key('subraces.no_subrace', $translations, translate_key('subraces.no_subrace', $translations)) }}
      @endif
    </p>
